...
assert('SmartRateLimit.can_access_if_at_the_begining when begining session') do
  ...
  redis.del("example.com_#{(Time.now - 60).min}")
  redis.del("example.com_#{Time.now.min}")
...
assert('SmartRateLimit.can_access_if_at_the_begining when over maxconnection') do
  ...
  11.times do |i|
    redis.sadd("example.com_#{Time.now.min}", i.to_s)
  end
  ...
  assert_nil redis.get('example.com_list_lock')
  assert_equal 11, redis.scard("example.com_#{Time.now.min}")
...
assert('SmartRateLimit.wait_list_length') do
  redis.del('example.com_waitlist')
  redis.lpush('example.com_waitlist', 'abcd')
  redis.lpush('example.com_waitlist', 'aefgh')
  assert_equal 2, subject.wait_list_length
end
